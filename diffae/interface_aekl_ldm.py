...
from .models.encoder import SemanticEncoder, SemanticEncoder3D
from .models.unet import Unet, Unet3D
from .models.model import DiffusionAutoEncoders3D
...
from diffae.trainer_aekl_ldm import train_ldm

class AEKLLDMInterface(DiffusionAutoEncodersInterface):
    def __init__(self, config, mode, ckpt_path=None, split_batches=True):
        ...
        self.aekl_model, self.diffusion, self.scheduler, self.train_days, self.start_time, self.steps = self._init_model(ckpt_path=ckpt_path)
        ...
        self._init_dataset()
        self.optimizer = optim.AdamW(self.diffusion.parameters(), lr=config["ldm"]["base_lr"])
    ...
    def _init_model(self, ckpt_path=None):
        ...
        assert self.config.three_d == True, "Only 3D models are supported at the moment"

        aekl_model = AutoencoderKL(**self.config.stage1.params).cuda()
        # if self.config.stage1.ckpt is not None:
        #     ckpt = torch.load('/home/peter/PycharmProjects/videoMRI/outputs/aekl_s3_l3_b8.pth')
        #     aekl_model.load_state_dict(ckpt["state_dict"], strict=True)
        #     aekl_model.eval()
        scheduler = DDPMScheduler(**self.config["ldm"].get("scheduler", dict())).cuda()
        diffusion = DiffusionAutoEncoders3D(self.config["semantic_encoder"], self.config["ldm"].get("dmunet", dict())).cuda()        
        # diffusion = Unet3D(**self.config["ldm"].get("dmunet", dict()))
        # diffusion = DiffusionModelUNet(**self.config["ldm"].get("dmunet", dict()))
        # semantic_encoder = SemanticEncoder3D(**self.config["semantic_encoder"])
        ...
        return aekl_model, diffusion, scheduler, train_days, start_time, steps
