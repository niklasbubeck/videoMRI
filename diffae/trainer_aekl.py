...
def train_epoch_aekl(
    ...
    for step, batch in pbar:
        ...
        epoch_l1_loss += l1_loss.item()/len(loader)
        epoch_p_loss += p_loss.item()/len(loader)
        epoch_kl_loss += kl_loss.item()/len(loader)
        epoch_g_loss += g_loss.item()/len(loader)
        epoch_d_loss += discriminator_loss.item()/len(loader)
...
def eval_aekl(
    ...
    # wandb.log(total_losses, step=step, commit=False)
    for k, v in total_losses.items():
        wandb.log({f"val/{k}": v}, step=epoch, commit=False)
    wandb.log({f"val/psnr": epoch_psnr}, step=epoch, commit=False)
    video_log = torch.cat([images, reconstruction], dim=-2)
    video_log = video_log[:,:,::2]
